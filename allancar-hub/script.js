...
car.addEventListener('load', (event) => {
  ...
  /* CHANGE CAR PAINT */
  paint.pbrMetallicRoughness.setBaseColorFactor('#f0627a');
  ...
  swiper.on('slideChange', function () {
    if ( swiper.activeIndex === 0 ) {
      gsap.to(car, carPosition(exposure1, orbit1, target1));
      page.classList.remove('bg-red-900');
      page.classList.add('bg-green-200');
    } else if ( swiper.activeIndex === 1 ) {
      gsap.to(car, carPosition(exposure2, orbit2, target2));
      page.classList.remove('bg-green-200');
      page.classList.add('bg-red-900');
    } else if ( swiper.activeIndex === 2 ) {
      gsap.to(car, carPosition(exposure3, orbit3, target3));
      page.classList.remove('bg-red-900');
      page.classList.add('bg-red-200');
    }
